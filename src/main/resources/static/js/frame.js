;(function(){
    ...
    $.ajax({
        type : "POST",
        url : '/coreMenu/queryAllMenu',
        dataType : "json",
        data : {},
        success : function(data){
            layer.closeAll();
            if (data.code == 200){
                initMenu(data.listData);
            } else {
                winFn.msg(data.msg);
                var error = [];
                initMenu(error);
    ...
    function createMenuItem(menu){
        if(menu.children != null){
            ...
            var pl = (menu.level - 1) * 1;
            return "<li menu-id='" + menu.id + "'>" + "<div class='box' style='padding-left: " + pl + "px'>" + "<i class='" + menu.icon + " menu-icon'></i>" + "<a>" + menu.title + "</a>" + "<i class='menu-arrow icon-sk027 " + arrowCls + "'></i>" + "</div>" + "</li>";
        }
    }
    /**
     * 响应菜单项的点击事件
    ...
    function excuteMenuEvent(ev){
        ...
        addTab({
            id: idMapData[id].id, title: idMapData[id].title, path: idMapData[id].path
        });
    }
    function addTab(tab){
    ...
    function createTab(tab){
        var tab = "<div class='tab open' tab-id='" + tab.id + "'>" + "<span>" + tab.title + "</span>" + "<i class='icon-sk021 tab-close' style='cursor:pointer;' title='关闭'></i>" + "</div>";
        return tab;
    }
    /**
     * 创建新的页面
    ...
    function createTabPage(id, path){
        var page = $("<div class='containerPage' id='page" + id + "' ></div>");
        if(currPage != null){
     ...
     * 关闭tab
     * 当前页面被关闭：
     *  1.默认显示左边第一个Tab,如果没有，则显示右边第一个，否则走步骤2；
     *  2.没有其他界面，则显示空白
     */
